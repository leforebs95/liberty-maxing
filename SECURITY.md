...
If you discover a security vulnerability in liberty-max, please report it by:
...
- ✅ Run liberty-max with a dedicated user account
...
- Keep authentication data in `~/.liberty-max/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade liberty-max-ai
```
...
- Subscribe to security advisories for liberty-max and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash liberty-max
   sudo -u liberty-max liberty-max gateway
   ```
...
4. **Enable Logging**
   ...
   tail -f ~/.liberty-max/logs/liberty-max.log
   ```
...
- **Chat history is stored locally** - protect the `~/.liberty-max` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.liberty-max/logs/liberty-max.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/liberty-max/security/advisories
- Release Notes: https://github.com/HKUDS/liberty-max/releases
